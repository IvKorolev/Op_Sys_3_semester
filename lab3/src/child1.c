...
int main() {
    ...
    if (shm_fd == -1) {
        const char msg[] = "error: failed to open shared memory in child1\n";
    ...
    if (shared_memory == MAP_FAILED) {
        const char msg[] = "error: failed to map shared memory in child1\n";
        write(STDERR_FILENO, msg, sizeof(msg) - 1);
        exit(EXIT_FAILURE);
    }
    ...
    if (sem_read == SEM_FAILED || sem_write == SEM_FAILED) {
        const char msg[] = "error: failed to open semaphores in child1\n";
        write(STDERR_FILENO, msg, sizeof(msg) - 1);
        exit(EXIT_FAILURE);
    }
